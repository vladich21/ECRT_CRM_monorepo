...
@Controller('partner-statuses')
export class PartnerStatusesController {
  constructor(private readonly service: PartnerStatusesService) {}
  ...
  async update(@Param('id') id: string, @Body('body') body?: Record<string, unknown>) {
    ...
    if (!row) throw new NotFoundException(`Статус ${id} не найден`);
    return [row];
  }

...
Шапка одной проведенной оценки (одна запись = один проход матрицы по контрагенту и проекту).
...
При появлении новой активной блокировки по той же паре политику (одна активная запись vs история) задает сервис; минимально — искать `is_active = true` для (`partner_id`, `project_id`).
...
- После расчета `weighted_score` назначается `category` по порогам (как в UI: фильтр по категориям):
  - `A`: балл **≥ 4.0**
...
- `next_reevaluation_date` от `evaluated_at`:
  ...
  - `B`: +6 мес.
  - `C`: +3 мес.
  - `D`: `null`, создается/обновляется блокировка (§4).
...
- `partners.rating`, `partners.next_audit_date` остаются; агрегат по всем проектам/последней оценке задается в приложении.

...
export const userUpdateFormMapper = (userData: User) => {
  const values = {
    ...
    department_id:
      userData.department?.id != null && userData.department.id !== '' ? String(userData.department.id) : undefined,
    position_id:
      userData.position?.id != null && userData.position.id !== '' ? String(userData.position.id) : undefined,

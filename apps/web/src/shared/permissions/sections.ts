/**
 * Зеркало backend-констант apps/backend-nest/src/shared/permissions/sections.ts.
 *
 * При добавлении нового раздела:
 *   1. Добавить запись в БД (sections) через миграцию или админку.
 *   2. Синхронно добавить в backend `SECTIONS` и здесь.
 *   3. Использовать в `<CanAccess>` / `usePermissions`.
 */

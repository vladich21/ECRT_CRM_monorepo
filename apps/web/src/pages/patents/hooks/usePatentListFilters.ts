...
export function usePatentListFilters() {
  ...
  const [appliedFilters, setAppliedFilters] = useState<PatentAdvancedFilters>(
    DEFAULT_PATENT_FILTERS
  );
  const [draftFilters, setDraftFilters] = useState<PatentAdvancedFilters>(
    DEFAULT_PATENT_FILTERS
  );

  const activeFiltersCount = countActivePatentFilters(appliedFilters);

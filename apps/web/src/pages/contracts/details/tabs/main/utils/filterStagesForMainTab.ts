...
import {
  ...
  getStageStatus,
} from '../../stages/utils/stageHelpers';
...
export function filterAndSortStagesForMainTab(
  ...
  const afterStatus =
    filters.status === 'all'
      ? afterSearch
      : afterSearch.filter(
          (stage) => getStageStatus(stage, contractStageStates).status === filters.status,
        );
  ...
  const afterResponsible =
    filters.responsible === 'all'
      ? afterStatus
      : afterStatus.filter(
          (stage) => String(stage.responsible_id || '') === String(filters.responsible),
        );
  ...
  const afterBudget =
    filters.budget === 'all'
      ...
      : afterDeadline.filter((stage) => {
          const deviation = calculateBudgetDeviation(
            stage.planned_budget ?? null,
            stage.actual_budget ?? null,
          );
  ...
  return [...afterBudget].sort(
    (stageA, stageB) => stageA.stage_number - stageB.stage_number,
  );
}

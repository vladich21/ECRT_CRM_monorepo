...
export function ContractMainInfoTab() {
  ...
  const toggleAllVisibleStagesExpanded = () => {
    ...
    setUi(previous => ({
      ...previous,
      expandedStageIds: applyBulkExpandCollapseForStages(filteredStages, previous.expandedStageIds),
    }));
  };
  ...
  const handleSaveStage = async () => {
    ...
    if (ui.editingStageId) {
      ...
      setStagesState(previous =>
        previous.map(stageRow => {
          ...
          const updated: ContractStage = {
            ...stageRow,
            name: values.name,
            responsible_id: values.responsible_id != null && values.responsible_id !== '' ? values.responsible_id : '',
      ...
      const maxStageNumber =
        (stagesState.reduce((maxNumber, stageRow) => Math.max(maxNumber, stageRow.stage_number), 0) || 0) + 1;
  ...
  return (
    <>
      <ContractMainInfoSummaryCard
        ...
        onToggleMainInfoExpanded={() => setUi(previous => ({ ...previous, isExpanded: !previous.isExpanded }))}
      ...
      >
        <ContractStagesToolbar
          ...
          onStageSearchChange={value => setUi(previous => ({ ...previous, stageSearch: value }))}
        ...
        {filteredStages.length === 0 && stagesState.length === 0 && <Text type='secondary'>Этапы не загружены.</Text>}

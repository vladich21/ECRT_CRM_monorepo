...
export function filterByTab(
  ...
  switch (tab) {
    ...
    case 'inactive':
      return contracts.filter(contract => !contract.is_active);
    case 'draft':
      return contracts.filter(contract => isContractDraft(contract.state_id, contractStates));
    default:
      return contracts;
  }
}
export function filterByAdvanced(contracts: Contract[], filters: AdvancedFilters): Contract[] {
  ...
  if (filters.dateRange?.[0] && filters.dateRange?.[1]) {
    const [fromDate, toDate] = [filters.dateRange[0].format('YYYY-MM-DD'), filters.dateRange[1].format('YYYY-MM-DD')];
    result = result.filter(
      contract => (contract.start_date || '') <= toDate && (contract.end_date || contract.start_date || '') >= fromDate,
    );
  }
  if (filters.amountMin != null) {
    result = result.filter(contract => (contract.amount_incl_vat ?? 0) >= filters.amountMin!);
  }
  if (filters.amountMax != null) {
    result = result.filter(contract => (contract.amount_incl_vat ?? 0) <= filters.amountMax!);
  }
  return result;
}
export function filterBySearch(
  ...
  return contracts.filter(contract => {
    const partnerName = references?.partners?.find(partner => partner.id === contract.partner_id)?.name ?? '';
    return (

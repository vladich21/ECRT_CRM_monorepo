...
describe('buildDocumentCreateSaveWarnings', () => {
  it('занятое обозначение — подсказка у обозначения', () => {
    const error = apiError({ code: 'DOCUMENT_TAKEN', field: 'designation', message: 'x', occupiedDesignation: 'A 13 01' });
  ...
  it('отказ с полем — подсказка у этого поля', () => {
    ...
    expect(buildDocumentCreateSaveWarnings(apiError({ code: 'FILE_NOT_READY', field: 'file', message: 'Ждём' }))).toEqual({
      file: 'Ждём',
    });
    expect(buildDocumentCreateSaveWarnings(apiError({ code: 'ITEM_ARCHIVED', field: 'form', message: 'Архив' }))).toEqual({
      form: 'Архив',
    });
  });

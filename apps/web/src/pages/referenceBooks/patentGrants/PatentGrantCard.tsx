...
export function PatentGrantCard({ grant, onClick, onEdit, onDelete }: Props) {
  ...
  const dateStr = [formatDate(grant.grant_date), formatDate(grant.renewal_date)]
    .filter(Boolean)
    .join(' — ') || '—';
  ...
  return (
    ...
    >
      <div className={styles.mainInfo}>
        ...
        <Tag color={statusColor} style={{ fontSize: 13 }}>{grant.status}</Tag>
        {grant.office && (
          <div className={styles.office}>{grant.office}</div>
        )}
      </div>
      ...
      <div className={styles.actionsCol}>
        ...
        <span
          ...
          onClick={(e) => { e.stopPropagation(); onDelete(grant); }}
        >
          Удалить
        </span>
      </div>
